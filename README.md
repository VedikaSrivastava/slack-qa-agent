...
and required LangSmith tracing plus versioned offline experiments.
...
Reviewers need only Git and Docker Desktop. Python, uv, PostgreSQL, SQLite CLI, Make, and the Inngest
...
On Windows PowerShell, use `Copy-Item .env.example .env.local` for the first command. Fill every
required value in `.env.local`; each process validates its own required configuration and fails
immediately when a value is missing or blank. `.env.local` is ignored by Git. Model names and action budgets are code-defined
in reviewed `AgentProfile` values, so environment variables cannot silently switch either one. The
current production profile is `balanced-gpt-4.1-mini` until a saved experiment justifies changing it.
...
checkpoint setup. Alembic owns only `agent_runs`, `run_sources`, and `feedback`; LangGraph owns its
checkpoint tables.
...
substitute for the Slack integration. It still requires the complete validated environment. To reuse
multi-turn state across CLI calls, pass the same `--conversation-id` value. Slack derives this value
from team ID, channel ID, and the root thread timestamp, so different threads are isolated and
follow-ups in one thread share state.
...
All settings are environment variables; `.env.example` contains placeholders only.

- `OPENAI_API_KEY`: model access; all selectable model names are fixed in code
- `SLACK_BOT_TOKEN`, `SLACK_SIGNING_SECRET`: required Slack transport credentials
- `POSTGRES_PASSWORD`: required password for the local Postgres container
- `DATABASE_URL`: async SQLAlchemy PostgreSQL URL
- `KNOWLEDGE_DB_PATH`: immutable SQLite knowledge file
- `INNGEST_DEV`, `INNGEST_BASE_URL`: local durable execution
- `LANGSMITH_TRACING=true`, `LANGSMITH_API_KEY`, `LANGSMITH_PROJECT=slack-qa-agent`: required
  tracing and experiment storage
- `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY`: required when `APP_ENV=production`
- `LOG_LEVEL`: structured JSON log level
- `PROMPT_VERSION`, `RETRIEVAL_VERSION`, `APP_VERSION`: trace and run-ledger metadata
...
- The supplied database has 250 artifacts and a standalone FTS5 index. Lexical artifact retrieval
  is complemented by a fixed, parameterized relational account lookup for bounded cross-account
  questions; no arbitrary SQL is model-accessible.
...
- Feedback persistence exists, but Slack feedback UX is not enabled.
