...
from slack_qa_agent.agent.profiles import EXPERIMENT_PROFILES, AgentProfile, get_experiment_profile
from slack_qa_agent.agent.service import create_question_processor
from slack_qa_agent.config import ExperimentSettings
from slack_qa_agent.evals.augmentation import generate_augmentation_candidates
...
from slack_qa_agent.evals.models import EvalCase, EvalResult
from slack_qa_agent.evals.protocols import EXPERIMENT_PROTOCOLS, get_experiment_protocol
...
def load_settings(env_file: Path | None) -> ExperimentSettings:
    if env_file is not None and not env_file.is_file():
        raise FileNotFoundError(f"Environment file does not exist: {env_file}")
    return ExperimentSettings(_env_file=env_file)


def langsmith_client(settings: ExperimentSettings) -> Client:
    return Client(api_key=settings.langsmith_api_key.get_secret_value())


async def run_suite(
    settings: ExperimentSettings, suite: str, profile: AgentProfile
) -> list[EvalResult]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m slack_qa_agent.evals")
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if args.command == "experiment":
        ...
        summary = await run_langsmith_experiment(
            client=client,
            settings=settings,
            cases=cases,
            profile=profile,
