...
from slack_qa_agent.agent.graph import build_graph
...
from slack_qa_agent.config import AgentSettings
from slack_qa_agent.retrieval.repository import SQLiteKnowledgeRepository


class LangGraphQuestionProcessor:
    def __init__(self, graph: Any, settings: AgentSettings, profile: AgentProfile) -> None:
        self._graph = graph
...
async def create_question_processor(
    settings: AgentSettings, profile: AgentProfile
    ...
    await anyio.to_thread.run_sync(repository.inspect_assignment_schema)
    tools = KnowledgeTools(repository)
    async with AsyncPostgresSaver.from_conn_string(settings.psycopg_database_url()) as checkpointer:
        graph = build_graph(AgentNodes(model, tools, profile), checkpointer=checkpointer)
